...
//     cudaStreamSynchronize(streams2); 
    ...
    for (int i = 0; i < numStream; i++)
    {
        ...
        if (i != numStream-1) {
            cudaMemcpyAsync(&A_d[Offset], &A_h[Offset], sizeof(float)*Offset, cudaMemcpyHostToDevice, streams[i]);
        ...
        if (i != numStream-1) {
            ...
            cudaMemcpyAsync(&C_h[Offset], &C_d[Offset], sizeof(float)*Offset, cudaMemcpyDeviceToHost, streams[i]);
        } else {
            Offset = (i * segmentLen) + VecSize % numStream;
